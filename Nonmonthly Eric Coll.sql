...
sum(case when schedulestat = '85' then 1 else 0 end) as Transmit,
...
--**REGION/PANEL Color Counts for Sparklines
--SELECT * FROM
--(SELECT blsreg, panel, display_color
--  FROM seg_sufficiency)
--PIVOT 
--( count(display_color)
--  FOR display_color 
--  in ('red', 'yellow', 'green')
--)
--order by blsreg, panel;
...
--SELECT * FROM
--(SELECT blsreg, psucoll, panel, zero, one, two, three, four, five
--  FROM seg_sufficiency2)
--PIVOT 
--( count(zero)
--  FOR zero
--  in ('0', '1', '2', '3', '4', '5')
...
sum(outstanding_Fielded) as outstanding,
sum(transmit) as transmitted,
case when sum(e.Good_Rent) > '0' then sum(e.good_rent) else null end as Rents,
...
sum(ki.klein_insuff_units) as klein_insuff_units,
...
GROUP BY e.BLSREG, E.psucoll, e.panel, e.dtcloseout,f160.suff160_units
...
a.fielded,
case when (a.transmitted) > 0 then (a.transmitted) else null end as transmitted,
a.outstanding,
a.unassigned,
a.total_klein,
a.total_needed,
...
SELECT 
coalesce(blsreg, 'National') as blsreg, 
'' as psucoll,
coalesce(panel, 'Overall') as panel, 
...
case when sum(a.transmitted) > 0 then sum(a.transmitted) else null end as transmitted,
